...
List<Ocorrencia> gerarOcorrencias(
  ...
  final chunks = <_SerieChunk>[
    ...
    ...serie.revisoes.map((r) => _SerieChunk(
  ...
  for (int i = 0; i < chunks.length; i++) {
    ...
    final end = (i + 1 < chunks.length) ? chunks[i + 1].dataEfeito.addDays(-1) : null;

    final dates = _generateDates(chunk.regra);
    for (final date in dates) {
      if (date.isBefore(start)) continue; // Pertence ao chunk anterior (ou irrelevante)
      if (end != null && date.isAfter(end)) break; // Terminou o chunk

      if (serie.dataEncerramento != null && date.isAfter(serie.dataEncerramento!)) {
        return ocorrencias.where((o) => o.dataVencimento.isSameOrAfter(desde) && o.dataVencimento.isSameOrBefore(ate)).toList();
      }
      ...
      if (date.isSameOrAfter(desde) && date.isSameOrBefore(ate)) {
        final dataFormatada = "${date.year.toString().padLeft(4, '0')}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}";
    ...
    while (true) {
      switch (regra.frequencia) {
        ...
        case Frequencia.mensal:
          ...
          int rawMonth = regra.dataInicial.month + (iter * regra.intervalo);
          int additionalYears = (rawMonth - 1) ~/ 12;
          int finalMonth = ((rawMonth - 1) % 12) + 1;
          int finalYear = regra.dataInicial.year + additionalYears;
          
          yield CivilDate(finalYear, finalMonth, regra.diaEfetivoNoMes(finalYear, finalMonth));
          break;
        case Frequencia.anual:
          int finalYear = regra.dataInicial.year + (iter * regra.intervalo);
          int finalMonth = regra.dataInicial.month;
          yield CivilDate(finalYear, finalMonth, regra.diaEfetivoNoMes(finalYear, finalMonth));
          break;
