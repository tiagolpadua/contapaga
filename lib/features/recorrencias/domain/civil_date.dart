...
class CivilDate implements Comparable<CivilDate> {
  ...
  CivilDate._validated(this.year, this.month, this.day) {
    if (day > _daysInMonth(year, month)) {
      throw ArgumentError('Dia $day não existe no mês $month de $year');
    }
  }

  factory CivilDate.validated(int year, int month, int day) =>
      CivilDate._validated(year, month, day);

  factory CivilDate.hoje(Clock clock) {
    final now = clock.now();
    return CivilDate(now.year, now.month, now.day);
  }

  factory CivilDate.fromDate(DateTime date) {
  ...
  CivilDate addYears(int n) {
    int newYear = year + n;
    int newDay = day;
    int maxDays = _daysInMonth(newYear, month);
    if (newDay > maxDays) {
      newDay = maxDays;
    }
