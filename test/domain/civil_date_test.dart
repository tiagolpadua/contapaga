...
void main() {
  group('CivilDate', () {
    ...
    test('CivilDate.hoje e fromDate', () {
      ...
      expect(d1, equals(CivilDate(2026, 9, 16)));

      final d2 = CivilDate.fromDate(DateTime(2026, 10, 5, 23, 59));
      expect(d2, equals(CivilDate(2026, 10, 5)));
    });

    test('addDays atravessando meses', () {
      final d1 = CivilDate(2026, 9, 30);
      expect(d1.addDays(1), equals(CivilDate(2026, 10, 1)));
      
      final d2 = CivilDate(2026, 2, 28);
      expect(d2.addDays(1), equals(CivilDate(2026, 3, 1)));

      final d3 = CivilDate(2024, 2, 28);
      expect(d3.addDays(1), equals(CivilDate(2024, 2, 29)));
      expect(d3.addDays(2), equals(CivilDate(2024, 3, 1)));
    });

    test('addMonths com cap de dias inexistentes', () {
      // 31/01 -> +1 mes -> 28/02
      final d1 = CivilDate(2026, 1, 31);
      expect(d1.addMonths(1), equals(CivilDate(2026, 2, 28)));

      // 31/01/2024 -> +1 mes -> 29/02
      final d2 = CivilDate(2024, 1, 31);
      expect(d2.addMonths(1), equals(CivilDate(2024, 2, 29)));

      // 30/11 -> +1 mes -> 30/12
      final d3 = CivilDate(2026, 11, 30);
      expect(d3.addMonths(1), equals(CivilDate(2026, 12, 30)));

      // 15/12 -> +1 mes -> 15/01/2027 (virada de ano)
      final d4 = CivilDate(2026, 12, 15);
      expect(d4.addMonths(1), equals(CivilDate(2027, 1, 15)));
    });

    test('addYears com regra 29/02', () {
      final d1 = CivilDate(2024, 2, 29);
      expect(d1.addYears(1), equals(CivilDate(2025, 2, 28)));
      expect(d1.addYears(4), equals(CivilDate(2028, 2, 29)));
    });

    test('comparações e ordenação', () {
      final d1 = CivilDate(2026, 9, 15);
      final d2 = CivilDate(2026, 9, 16);
      final d3 = CivilDate(2026, 10, 1);
