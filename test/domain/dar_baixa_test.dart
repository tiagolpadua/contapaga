...
void main() {
  group('dar_baixa e reverter_baixa', () {
    final clock = FakeClock(DateTime(2026, 9, 16, 12, 0));
    
    final o = Ocorrencia(
      serieId: 's1',
      id: 's1#1',
      dataVencimento: CivilDate(2026, 9, 10),
      valorPrevisto: Money(100),
      sequencia: 1,
    );
    ...
    test('baixa com sucesso', () {
      final baixada = darBaixa(
        o,
        valorPago: Money(120),
        dataPagamento: CivilDate(2026, 9, 11),
        clock: clock,
      );
      
      expect(baixada.baixa, isNotNull);
      expect(baixada.baixa!.valorPago, equals(Money(120)));
      expect(baixada.baixa!.dataPagamento, equals(CivilDate(2026, 9, 11)));
    ...
    test('reverter baixa', () {
      ...
      expect(revertida.status(clock), equals(StatusOcorrencia.atrasada)); // 10/09 < 16/09
    ...
    test('bloqueia baixa em ocorrencia já baixada', () {
      ...
      expect(
        () => darBaixa(baixada, valorPago: Money(10), dataPagamento: CivilDate(2026, 9, 11), clock: clock),
        throwsStateError,
      );
    });
    ...
    test('bloqueia data de pagamento no futuro', () {
      expect(
        () => darBaixa(o, valorPago: Money(10), dataPagamento: CivilDate(2026, 9, 17), clock: clock),
        throwsArgumentError,
      );
    });
    ...
    test('bloqueia valor pago zero ou negativo', () {
      expect(
        () => darBaixa(o, valorPago: Money(0), dataPagamento: CivilDate(2026, 9, 16), clock: clock),
        throwsArgumentError,
      );
      expect(
        () => darBaixa(o, valorPago: Money(-10), dataPagamento: CivilDate(2026, 9, 16), clock: clock),
        throwsArgumentError,
      );
    });
