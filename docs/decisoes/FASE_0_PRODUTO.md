...
- Lista agrupada, settlement em formulário (1d), confirmação de reversão (1j), navegação mensal por setas (1g), histórico de seis meses, edição, encerramento e ajustes/ajuda/privacidade.
...
| Tema | Decisão |
| --- | --- |
| Datas e competência | Datas civis completas; competência pelo year/mês de vencimento. Settlement posterior permanece na competência original. Uma série pode ter várias ocorrências no mês. |
...
| Dias 29–31 | Ajustar ao último dia do mês quando necessário, explicando a rule no cadastro; preservar o dia original da série nos meses seguintes. |
| Settlement | Uma settlement integral por ocorrência, valor positivo em centavos, editável e sem pagamentos parciais. Permitir data passada e antecipação; bloquear data de settlement futura e duplicidade. |
| Reversão | Confirmar antes de desfazer; ao cancelar, manter settlement. Ao confirmar, reabrir a mesma ocorrência e recalcular totais/status. |
| Atraso | Comparar datas completas; pagamento em mês posterior continua em atraso. |
| Débito automático | Apenas intenção de agendamento; exige settlement manual. Após vencer sem settlement, fica pendente em atraso. |
...
| Painel interno | Referente ao mês selecionado, inclui vencidas, hoje e próximos, inclusive automáticos sem settlement. Antecedência padrão de três dias, configurável de um a dez. |

## Recorrências e convenções de implementação

- Frequência diária, semanal, mensal ou anual, com interval inteiro positivo N (a cada N unidades). Valor sempre por ocorrência; não dividir automaticamente valor total.
...
- Edição de frequência cria revisão prospectiva da série. Ocorrências anteriores ao corte e baixadas mantêm identidade e dados; abertas afetadas são reconciliadas sem duplicação. Limite por quantidade vale para a série toda: ocorrências preservadas consomem a contagem; ao alterar a quantidade, mostrar total e restantes. Bloquear total menor que a quantidade preservada. Não oferecer exceções do type “terceira terça-feira” ou integração Google Calendar nesta v1.
...
- Atualizar agendamentos após settlement, reversão, edição, encerramento, importação e alteração de preferências; reconciliar na retomada e em eventos suportados pelo sistema. Evitar duplicações após reinício.
...
- Settlement e reversão: dar settlement na despesa de R$ 200,00 por R$ 220,00 reduz a pagar para zero e o saldo previsto para R$ 2.670,00. Cancelar a confirmação de reversão mantém esses números. Confirmar a reversão restaura a pagar de R$ 200,00 e saldo de R$ 2.690,00.
- Previsão mensal de setembro: baixas de abril R$ 100,00, junho R$ 120,00 e agosto R$ 110,00 produzem R$ 110,00. Meses sem settlement não entram como zero; settlement de setembro e baixas anteriores a março não entram. Sem baixas entre março e agosto, usar o valor base.
...
- Automático: vencimento em 10/09/2026 sem settlement está pendente em atraso em 11/09/2026, sem inferir pagamento.
- Validação: data de settlement posterior a hoje, valor zero/negativo e segunda settlement da mesma ocorrência não gravam dados.
...
- Diária: início em 30/09/2026, interval 1 e três ocorrências de R$ 10,00 gera 30/09, 01/10 e 02/10; totais previstos de R$ 10,00 em setembro e R$ 20,00 em outubro.
- Semanal: início 14/09/2026, segunda e quarta, interval de duas semanas e quatro ocorrências gera 14/09, 16/09, 28/09 e 30/09. Término em 28/09, em vez de quantidade, inclui essa data e exclui 30/09.
- Mensal: R$ 100,00 com 12 ocorrências resulta em R$ 1.200,00 previstos, com settlement individual em cada ocorrência.
...
- Edição: alteração com efeito em 20/09 preserva ocorrência de 19/09 e qualquer settlement de 21/09; mudança que removeria a ocorrência baixada de 21/09 é bloqueada.
- Notificação: hoje 30/09 e antecedência três dias inclui pendências de 01/10 a 03/10, além de hoje e vencidas, mesmo com agosto aberto no app; uma settlement antes do aviso retira a ocorrência do resumo agendado.
